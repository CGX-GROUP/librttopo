...
int rt_arc_calculate_gbox_cartesian_2d(const POINT2D *A1, const POINT2D *A2, const POINT2D *A3, GBOX *gbox)
{
	...
	if ( A2_side == rt_segment_side(A1, A3, &xmin) )
		gbox->xmin = xmin.x;

	if ( A2_side == rt_segment_side(A1, A3, &ymin) )
		gbox->ymin = ymin.y;

	if ( A2_side == rt_segment_side(A1, A3, &xmax) )
		gbox->xmax = xmax.x;

	if ( A2_side == rt_segment_side(A1, A3, &ymax) )
		gbox->ymax = ymax.y;
...
static int rtcircstring_calculate_gbox_cartesian(RTCIRCSTRING *curve, GBOX *gbox)
{
	...
	for ( i = 2; i < curve->points->npoints; i += 2 )
	{
		...
		if (rt_arc_calculate_gbox_cartesian(&p1, &p2, &p3, &tmp) == RT_FAILURE)
			continue;
...
static int rtcollection_calculate_gbox_cartesian(RTCOLLECTION *coll, GBOX *gbox)
{
	...
	{
		if ( rtgeom_calculate_gbox_cartesian((RTGEOM*)(coll->geoms[i]), &subbox) == RT_SUCCESS )
		{
...
int rtgeom_calculate_gbox_cartesian(const RTGEOM *rtgeom, GBOX *gbox)
{
	...
	rterror("unsupported type (%d) - %s", rtgeom->type, rttype_name(rtgeom->type));
	return RT_FAILURE;
}
