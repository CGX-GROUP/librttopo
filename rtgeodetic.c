...
static int gbox_check_poles(GBOX *gbox)
{
	...
	RTDEBUGF(4, "gbox %s", gbox_to_string(gbox));
	/* Z axis */
...
int clairaut_cartesian(const POINT3D *start, const POINT3D *end, GEOGRAPHIC_POINT *g_top, GEOGRAPHIC_POINT *g_bottom)
{
	...
	cart2geog(&t1, &vN1);
	cart2geog(&t2, &vN2);
	g_top->lat = z_to_latitude(t1.z,RT_TRUE);
	g_top->lon = vN2.lon;
	g_bottom->lat = z_to_latitude(t2.z,RT_FALSE);
...
int edge_intersection(const GEOGRAPHIC_EDGE *e1, const GEOGRAPHIC_EDGE *e2, GEOGRAPHIC_POINT *g)
{
	...
	RTDEBUGF(4, "fabs(dot_product(ea, eb)) == %.14g", fabs(dot_product(&ea, &eb)));
	if ( FP_EQUALS(fabs(dot_product(&ea, &eb)), 1.0) )
	{
		RTDEBUGF(4, "parallel edges found! dot_product = %.12g", dot_product(&ea, &eb));
...
int edge_calculate_gbox(const POINT3D *A1, const POINT3D *A2, GBOX *gbox)
{
	...
	{
		rterror("Antipodal (180 degrees long) edge detected!");
		return RT_FAILURE;
	}
	...
	o_side = rt_segment_side(&R1, &R2, &O);
	...
	for ( i = 0; i < 6; i++ )
	{
		...
		if ( rt_segment_side(&R1, &R2, &RX) != o_side )
		{
			POINT3D Xn;
...
void gbox_pt_outside(const GBOX *gbox, POINT2D *pt_outside)
{
	...
	rterror("BOOM! Could not generate outside point!");
	return;
}
...
ptarray_segmentize_sphere(const POINTARRAY *pa_in, double max_seg_length)
{
	...
	if ( ! pa_in )
		rterror("ptarray_segmentize_sphere: null input pointarray");
	if ( max_seg_length <= 0.0 )	
		rterror("ptarray_segmentize_sphere: maximum segment length must be positive");
	...
	ptarray_append_point(pa_out, &p1, RT_FALSE);
	...
	while ( pa_in_offset < pa_in->npoints )
	{
		...
		if ( d > max_seg_length )
		{
			...
			for ( i = 0; i < nsegs - 1; i++ )
			{
				...
				if ( hasm )
					p.m += dmm;
				ptarray_append_point(pa_out, &p, RT_FALSE);
			}
			
			ptarray_append_point(pa_out, &p2, RT_FALSE);
		}
		/* This edge is already short enough */
		else
		{
			ptarray_append_point(pa_out, &p2, (pa_in->npoints==2)?RT_TRUE:RT_FALSE);
		}
...
rtgeom_segmentize_sphere(const RTGEOM *rtg_in, double max_seg_length)
{
	...
	/* Clone empty */
	if ( rtgeom_is_empty(rtg_in) )
		return rtgeom_clone(rtg_in);
	...
	case RTPOLYGONTYPE:
		...
		{
			pa_out = ptarray_segmentize_sphere(rtpoly_in->rings[i], max_seg_length);
			rtpoly_add_ring(rtpoly_out, pa_out);
		}
	...
	case RTCOLLECTIONTYPE:
		...
		{
			rtcollection_add_rtgeom(rtcol_out, rtgeom_segmentize_sphere(rtcol_in->geoms[i], max_seg_length));
		}
		return rtcollection_as_rtgeom(rtcol_out);
		break;
	default:
		rterror("rtgeom_segmentize_sphere: unsupported input geometry type: %d - %s",
		        rtg_in->type, rttype_name(rtg_in->type));
		break;
	}
	
	rterror("rtgeom_segmentize_sphere got to the end of the function, should not happen");
	return NULL;
}
...
double rtgeom_area_sphere(const RTGEOM *rtgeom, const SPHEROID *spheroid)
{
	...
	if ( type == RTMULTIPOLYGONTYPE || type == RTCOLLECTIONTYPE )
	{
		...
		{
			area += rtgeom_area_sphere(col->geoms[i], spheroid);
		}
		return area;
	}
...
RTPOINT* rtgeom_project_spheroid(const RTPOINT *r, const SPHEROID *spheroid, double distance, double azimuth)
{
	...
	{
		rterror("Azimuth must be between -2PI and 2PI");
		return NULL;
	}
	...
	{
		rterror("Distance must be between 0 and %g", M_PI * spheroid->radius);
		return NULL;
	}
	...
	/* Try the projection */
	if( spheroid_project(&geo_source, spheroid, distance, azimuth, &geo_dest) == RT_FAILURE ) 
	{
		RTDEBUGF(3, "Unable to project from (%g %g) with azimuth %g and distance %g", x, y, azimuth, distance);
		rterror("Unable to project from (%g %g) with azimuth %g and distance %g", x, y, azimuth, distance);
		return NULL;
	}
	...
	rtp = rtpoint_construct(r->srid, NULL, pa);
	rtgeom_set_geodetic(rtpoint_as_rtgeom(rtp), RT_TRUE);
	return rtp;
}
...
* 
*/
double rtgeom_azumith_spheroid(const RTPOINT *r, const RTPOINT *s, const SPHEROID *spheroid)
{
	GEOGRAPHIC_POINT g1, g2;
	double x1, y1, x2, y2;
...
*/
double rtgeom_distance_spheroid(const RTGEOM *rtgeom1, const RTGEOM *rtgeom2, const SPHEROID *spheroid, double tolerance)
{
	uint8_t type1, type2;
	...
	if ( rtgeom_is_empty(rtgeom1) || rtgeom_is_empty(rtgeom2) )
	{
		return -1.0;
	}
	...
	if ( rtgeom1->bbox )
		...
		rtgeom_calculate_gbox_geodetic(rtgeom1, &gbox1);

	/* Make sure we have boxes */
	if ( rtgeom2->bbox )
		gbox2 = *(rtgeom2->bbox);
	else
		rtgeom_calculate_gbox_geodetic(rtgeom2, &gbox2);
	     ...
	     ( type2 == RTPOLYGONTYPE && type1 == RTPOINTTYPE ) )
	{
		...
		p = getPoint2d_cp(rtpt->point, 0);

		/* Point in polygon implies zero distance */
		if ( rtpoly_covers_point2d(rtpoly, p) )
		{
			return 0.0;
		}
		...
		{
			double ring_distance = ptarray_distance_spheroid(rtpoly->rings[i], rtpt->point, spheroid, tolerance, check_intersection);
	     ...
	     ( type2 == RTPOLYGONTYPE && type1 == RTLINETYPE ) )
	{
		...
		p = getPoint2d_cp(rtline->points, 0);
		...
		for ( i = 0; i < rtpoly->nrings; i++ )
		{
			double ring_distance = ptarray_distance_spheroid(rtpoly->rings[i], rtline->points, spheroid, tolerance, check_intersection);
	     ...
	     ( type2 == RTPOLYGONTYPE && type1 == RTPOLYGONTYPE ) )
	{
		...
		p = getPoint2d_cp(rtpoly1->rings[0], 0);
		if ( rtpoly_covers_point2d(rtpoly2, p) )
			return 0.0;

		/* Point of 1 in polygon 2 implies zero distance */
		p = getPoint2d_cp(rtpoly2->rings[0], 0);
		if ( rtpoly_covers_point2d(rtpoly1, p) )
			return 0.0;
		...
		for ( i = 0; i < rtpoly1->nrings; i++ )
		{
			...
			{
				double ring_distance = ptarray_distance_spheroid(rtpoly1->rings[i], rtpoly2->rings[j], spheroid, tolerance, check_intersection);
				if ( ring_distance < distance )
					distance = ring_distance;
				if ( distance < tolerance )
					return distance;
	...
	if ( rttype_is_collection(type1) )
	{
		...
		for ( i = 0; i < col->ngeoms; i++ )
		{
			double geom_distance = rtgeom_distance_spheroid(col->geoms[i], rtgeom2, spheroid, tolerance);
	...
	if ( rttype_is_collection(type2) )
	{
		...
		for ( i = 0; i < col->ngeoms; i++ )
		{
			double geom_distance = rtgeom_distance_spheroid(rtgeom1, col->geoms[i], spheroid, tolerance);
			if ( geom_distance < distance )
				distance = geom_distance;
			if ( distance < tolerance )
				return distance;
	...
	rterror("arguments include unsupported geometry type (%s, %s)", rttype_name(type1), rttype_name(type1));
	return -1.0;
...
int rtgeom_covers_rtgeom_sphere(const RTGEOM *rtgeom1, const RTGEOM *rtgeom2)
{
	...
	if ( rttype_is_collection( type1 ) )
	{
		...
		for ( i = 0; i < col->ngeoms; i++ )
		{
			if ( rtgeom_covers_rtgeom_sphere(col->geoms[i], rtgeom2) )
			{
				return RT_TRUE;
			}
		}
	...
	if ( rttype_is_collection( type2 ) )
	{
		...
		for ( i = 0; i < col->ngeoms; i++ )
		{
			if ( ! rtgeom_covers_rtgeom_sphere(rtgeom1, col->geoms[i]) )
			{
				return RT_FALSE;
			}
		}
	...
	/* Don't get here */
	rterror("rtgeom_covers_rtgeom_sphere: reached end of function without resolution");
	return RT_FALSE;
...
int rtpoly_covers_point2d(const RTPOLY *poly, const POINT2D *pt_to_test)
{
	...
	if ( ! poly || rtgeom_is_empty((RTGEOM*)poly) )
	{
	...
	RTDEBUGF(4, "polygon %s", rtgeom_to_ewkt((RTGEOM*)poly));
	RTDEBUGF(4, "gbox %s", gbox_to_string(&gbox));
...
static int rtpolygon_calculate_gbox_geodetic(const RTPOLY *poly, GBOX *gbox)
{
	...
	{
		if ( ptarray_calculate_gbox_geodetic(poly->rings[i], &ringbox) == RT_FAILURE )
			return RT_FAILURE;
		if ( first )
		{
...
static int rtcollection_calculate_gbox_geodetic(const RTCOLLECTION *coll, GBOX *gbox)
{
	...
	{
		if ( rtgeom_calculate_gbox_geodetic((RTGEOM*)(coll->geoms[i]), &subbox) == RT_SUCCESS )
		{
			/* Keep a copy of the sub-bounding box for later */
			if ( coll->geoms[i]->bbox ) 
				rtfree(coll->geoms[i]->bbox);
...
int rtgeom_calculate_gbox_geodetic(const RTGEOM *geom, GBOX *gbox)
{
	...
	case RTCOLLECTIONTYPE:
		...
		break;
	default:
		rterror("rtgeom_calculate_gbox_geodetic: unsupported input geometry type: %d - %s",
...
static int rtpoint_check_geodetic(const RTPOINT *point)
{
	assert(point);
	return ptarray_check_geodetic(point->point);
}

static int rtline_check_geodetic(const RTLINE *line)
{
	assert(line);
	return ptarray_check_geodetic(line->points);
}

static int rtpoly_check_geodetic(const RTPOLY *poly)
{
	int i = 0;
	assert(poly);
	...
	for ( i = 0; i < poly->nrings; i++ )
	{
		if ( ptarray_check_geodetic(poly->rings[i]) == RT_FALSE )
			return RT_FALSE;
	}
	return RT_TRUE;
}

static int rttriangle_check_geodetic(const RTTRIANGLE *triangle)
{
	assert(triangle);
...
static int rtcollection_check_geodetic(const RTCOLLECTION *col)
{
	int i = 0;
	assert(col);
	...
	for ( i = 0; i < col->ngeoms; i++ )
	{
		if ( rtgeom_check_geodetic(col->geoms[i]) == RT_FALSE )
			return RT_FALSE;
	}
	return RT_TRUE;
}

int rtgeom_check_geodetic(const RTGEOM *geom)
{
...
static int rtpoint_force_geodetic(RTPOINT *point)
{
	assert(point);
	return ptarray_force_geodetic(point->point);
}

static int rtline_force_geodetic(RTLINE *line)
{
	assert(line);
	return ptarray_force_geodetic(line->points);
}

static int rtpoly_force_geodetic(RTPOLY *poly)
{
	int i = 0;
	int changed = RT_FALSE;
	assert(poly);
	...
	for ( i = 0; i < poly->nrings; i++ )
	{
		if ( ptarray_force_geodetic(poly->rings[i]) == RT_TRUE )
			changed = RT_TRUE;
	}
	return changed;
}

static int rtcollection_force_geodetic(RTCOLLECTION *col)
{
	int i = 0;
	int changed = RT_FALSE;
	assert(col);
	...
	for ( i = 0; i < col->ngeoms; i++ )
	{
		if ( rtgeom_force_geodetic(col->geoms[i]) == RT_TRUE )
			changed = RT_TRUE;
	}
	return changed;
}

int rtgeom_force_geodetic(RTGEOM *geom)
{
...
double rtgeom_length_spheroid(const RTGEOM *geom, const SPHEROID *s)
{
	int type;
	int i = 0;
	double length = 0.0;
	...
	if ( rttype_is_collection( type ) )
	{
		...
		for ( i = 0; i < col->ngeoms; i++ )
		{
			length += rtgeom_length_spheroid(col->geoms[i], s);
		}
		return length;
	}

	rterror("unsupported type passed to rtgeom_length_sphere");
	return 0.0;
}
...
ptarray_nudge_geodetic(POINTARRAY *pa)
{
	...
	if ( ! pa )
		rterror("ptarray_nudge_geodetic called with null input");
...
*/
int 
rtgeom_nudge_geodetic(RTGEOM *geom)
{
	int type;
	int i = 0;
	int rv = RT_FALSE;
	...
	if ( rttype_is_collection( type ) )
	{
		...
		for ( i = 0; i < col->ngeoms; i++ )
		{
			int n = rtgeom_nudge_geodetic(col->geoms[i]);
			rv = (rv == RT_TRUE ? rv : n);
		}
		return rv;
	}

	rterror("unsupported type (%s) passed to rtgeom_nudge_geodetic", rttype_name(type));
	return rv;
}
