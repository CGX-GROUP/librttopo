...
circ_tree_free(CIRC_NODE* node)
{
	...
	if ( node->nodes ) rtfree(node->nodes);
	rtfree(node);
}
...
circ_node_internal_new(CIRC_NODE** c, int num_nodes)
{
	...
	for ( i = 1; i < num_nodes; i++ )
	{
		...
		else if ( ! rttype_is_collection(new_geom_type) )
		{
			...
			{
				new_geom_type = rttype_get_collectiontype(new_geom_type);				
			}
		}
		...
		{	
			...
			if ( circ_center_spherical(&c1, &(c[i]->center), dist, offset1, &new_center) == RT_FAILURE )
			{
...
circ_tree_new(const POINTARRAY* pa)
{
	...
	nodes = rtalloc(sizeof(CIRC_NODE*) * pa->npoints);
	j = 0;
...
circ_nodes_merge(CIRC_NODE** nodes, int num_nodes)
{
	...
	while( num_children > 1 )
	{
		for ( j = 0; j < num_children; j++ )
		{
			...
			if ( inode_num == 0 )
				inodes = rtalloc(sizeof(CIRC_NODE*)*CIRC_NODE_SIZE);
...
*/
	...
	{	
		...
		/* tests above. */
		if ( n1->geom_type && rttype_is_collection(n1->geom_type) )
		{
		...
		else if ( n2->geom_type && rttype_is_collection(n2->geom_type) )
		{
...
static CIRC_NODE*
rtpoint_calculate_circ_tree(const RTPOINT* rtpoint)
{
	CIRC_NODE* node;
	node = circ_tree_new(rtpoint->point);
	node->geom_type = rtgeom_get_type((RTGEOM*)rtpoint);;
	return node;
}

static CIRC_NODE*
rtline_calculate_circ_tree(const RTLINE* rtline)
{
	CIRC_NODE* node;
	node = circ_tree_new(rtline->points);
	node->geom_type = rtgeom_get_type((RTGEOM*)rtline);
	return node;
}

static CIRC_NODE*
rtpoly_calculate_circ_tree(const RTPOLY* rtpoly)
{
	int i = 0, j = 0;
	CIRC_NODE** nodes;
	CIRC_NODE* node;
	...
	if ( rtpoly->nrings == 1 )
	{
		node = circ_tree_new(rtpoly->rings[0]);			
	}
	else
	{
		/* Calculate a tree for each non-trivial ring of the polygon */
		nodes = rtalloc(rtpoly->nrings * sizeof(CIRC_NODE*));
		for ( i = 0; i < rtpoly->nrings; i++ )
		{
			node = circ_tree_new(rtpoly->rings[i]);
			if ( node )
				nodes[j++] = node;
		}
	...
	node->geom_type = rtgeom_get_type((RTGEOM*)rtpoly);
	rtpoly_pt_outside(rtpoly, &(node->pt_outside));
	
	return node;
}

static CIRC_NODE*
rtcollection_calculate_circ_tree(const RTCOLLECTION* rtcol)
{
	int i = 0, j = 0;
	CIRC_NODE** nodes;
	CIRC_NODE* node;
	...
	/* One geometry? Done! */
	if ( rtcol->ngeoms == 1 )
		return rtgeom_calculate_circ_tree(rtcol->geoms[0]);	
	
	/* Calculate a tree for each sub-geometry*/
	nodes = rtalloc(rtcol->ngeoms * sizeof(CIRC_NODE*));
	for ( i = 0; i < rtcol->ngeoms; i++ )
	{
		node = rtgeom_calculate_circ_tree(rtcol->geoms[i]);
		if ( node )
			nodes[j++] = node;
	}
	/* Put the trees into a spatially correlated order */
	...
	/* Don't need the working list any more */
	rtfree(nodes);
	node->geom_type = rtgeom_get_type((RTGEOM*)rtcol);
	return node;
}

CIRC_NODE*
rtgeom_calculate_circ_tree(const RTGEOM* rtgeom)
{
