...
static double ptarray_area_spheroid(const POINTARRAY *pa, const SPHEROID *spheroid)
{
	...
	if ( i != pa->npoints - 1 )
	{
		rterror("ptarray_area_spheroid: different number of points %d vs %d",
				i, pa->npoints - 1);
	}
...
double spheroid_distance(const GEOGRAPHIC_POINT *a, const GEOGRAPHIC_POINT *b, const SPHEROID *spheroid)
{
	...
	{
		rterror("spheroid_distance returned NaN: (%.20g %.20g) (%.20g %.20g) a = %.20g b = %.20g",a->lat, a->lon, b->lat, b->lon, spheroid->a, spheroid->b);
...
static double ptarray_area_spheroid(const POINTARRAY *pa, const SPHEROID *spheroid)
{
	...
	if ( signum(gbox2d.ymin) != signum(gbox2d.ymax) )
		rterror("ptarray_area_spheroid: cannot handle ptarray that crosses equator");
	...
	for ( i = 1; i < pa->npoints; i++ )
	{
		...
		RTDEBUGF(4, "crosses_dateline(a, b) %d", crosses_dateline(&a, &b) );
...
*/
double rtgeom_area_spheroid(const RTGEOM *rtgeom, const SPHEROID *spheroid)
{
	int type;
	...
	if ( type == RTMULTIPOLYGONTYPE || type == RTCOLLECTIONTYPE )
	{
		...
		{
			area += rtgeom_area_spheroid(col->geoms[i], spheroid);
		}
		return area;
	}
